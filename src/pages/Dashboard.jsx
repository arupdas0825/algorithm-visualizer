            ...
            <div className="flex flex-col gap-4">
              {aiRecs.map((rec, i) => (
                <motion.div 
                  key={i}
                  whileHover={{ x: 5 }}
                  className="p-5 bg-white/5 rounded-2xl border border-white/5 hover:bg-white/10 transition-all cursor-pointer group"
                  onClick={() => navigate(rec.path)}
                >
                  <div className="flex justify-between items-start mb-2">
                    <span className={`text-[10px] font-black uppercase tracking-wider ${rec.color}`}>{rec.type}</span>
                    <TrendingUp size={14} className="text-white/20 group-hover:text-white/60 transition-colors" />
                  </div>
                  <h4 className="font-bold text-white group-hover:text-primary transition-colors">{rec.title}</h4>
                  <p className="text-xs text-white/40 mt-1 leading-relaxed">{rec.desc}</p>
                </motion.div>
              ))}
            </div>
          </div>
          ...
          <div className="glass-panel p-8 rounded-[2.5rem] border border-accent/20 bg-accent/5">
            ...
            <div className="flex items-center gap-3">
              <div className="flex-1 h-2 bg-white/5 rounded-full overflow-hidden">
                <div className="h-full w-1/2 bg-accent shadow-[0_0_10px_rgba(6,182,212,0.5)]" />
              </div>
              <span className="text-xs font-bold text-white/40">1/2</span>
            </div>
          </div>
